...
- Keep tickets, modules, and architecture rules behind replaceable engine-owned ports.
- Keep business logic out of `src/direct.ts`, `src/cli.ts`, `src/mcp-stdio.ts`, `src/mcp-http.ts`.
- Keep HTTP authentication fail-closed: `src/http-auth.ts` verifies `ENGINEERING_ENGINE_HTTP_TOKEN`.
